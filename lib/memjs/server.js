...
Server.prototype.respond = function(response) {
  ...
  var callback = this.responseCallbacks[1];
  ...
  callback(response);
  if (!callback.quiet) {
    delete(this.responseCallbacks[1]);
    this.requestTimeouts.shift();
    delete(this.errorCallbacks[1]);
  }
};
...
Server.prototype.responseHandler = function(dataBuf) {
  ...
  // console.log(this.responseBuffer)
      this.respond(this.responseBuffer);

  return  parseMessage(dataBuf)
...
var timeoutHandler = function(server, sock) {
  console.log(server.requestTimeouts)
