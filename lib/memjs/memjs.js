...
Client.prototype.get = function(key, callback) {
  ...
  this.perform(key, request, `O${this.seq.toString()}-mg`, function(err, response) {
...
Client.prototype.gets = function(key, callback) {
  ...
  this.perform(key, request, `O${this.seq.toString()}-mgs`, function(err, response) {
    if (err) {
      if (callback) { callback(err, null, null); }
      return;
    }
...
Client.prototype.set = function(key, value, options, callback) {
  ...
  let request = `ms ${key} ${value.length} f v O${this.seq}\r\n${value}\r\n`
  this.perform(key, request, `O${this.seq}`, function(err, response) {
    if (err) {
...
Client.prototype.perform = function(key, request, seq, callback, retries, func) {
  var _this = this;
  ...
  var errorHandler = function(error) {
    if (--retries > 0) {
      ...
      setTimeout(function() {
        _this.perform(key, request, seq, callback, retries, func);
      }, 1000 * retry_delay);
    } else {
      ...
      if (failover) {
        ...
        _this.perform(key, request, seq, callback, origRetries, func);
      } else {
