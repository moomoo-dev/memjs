...
// - C(token): check CAS, do not send value if matches
...
Client.prototype.get = function(key, callback) {
  ...
  // let request = `get ${key} Oopaque\r\n`
  let request = `mg ${key} f k v O${this.seq.toString()}-mg\r\n`

  this.perform(key, request, `O${this.seq.toString()}-mg`, "get", function(err, response) {
    if (err) {
    ...
    case  0:
      if (callback) {
        ...
        callback(null, response.data, response.extras);
      }
      break;
    // case  "EN":
    case 1:
...
Client.prototype.gets = function(key, callback) {
  ...
  if(callback === undefined) {
    return promisify(function(callback) {
      self.gets(key, function(err, value, flags) {
        callback(err, value);
      });
    });
  }
  ...
  this.incrSeq();
  let request = "";
  key.forEach( e => { request += `mg ${e.trim()} f k v O${this.seq.toString()}-mgs\r\n`})

  // console.log(request)
  this.perform(key, request, `O${this.seq.toString()}-mgs`, "get",  function(err, response) {
    if (err) {
      if (callback) { callback(err, null, null); }
      return;
    }
    ...
    //   break;
    default:
      var errorMessage = 'MemJS GETS: ' + errors[response.header.status];
      logger.log(errorMessage);
      if (callback) { callback(new Error(errorMessage), null, null); }
    }
  });
...
// if (!String.prototype.makeid_no_special_characteres) {

  makeid_no_special_characteres = function(length) {
    let result = '';
...
// // Configuration
// process.env.MEMCACHIER_SERVERS = `${process.env.MEMCACHED_CONTAINER_NAME}:11211`;
const net = require('net');
...
Client.prototype.set = function(key, value, options, callback) {
  ...
  // let request = `set ${key} 0 3600 ${value.length}\r\n${value}`
  // let request = `get ${key}\r\n`
  // let request = `set ${key} 0 3600 ${value.length}\r\n${value}\r\n`
  // const seq = `O${makeid_no_special_characteres(10).toString()}`
  // console.log(value)
  // console.log(`key: ${key} length: ${value.length}`)
// let seq = 1;

  let request = 'ms '+ key + ' ' + value.length + ' O' + this.seq+ ' f v \r\n' + value+ '\r\n';
  // console.log(request)
  // let request = `ms ${key} ${value.toString().length} O${this.seq} f v \r\n`+ value+ "\r\n";
  // let command = 'ms ua_in_progress_10a47d7b-6544-47a1-9ea4-d5e3d969ff46_self-regulated-learning '+ value2.length + ' O' + seq +' f v \r\n' + value2 + '\r\n';

  // console.log(`value.length: ${value.length/1024}`)
  // console.log(request)
  this.perform(key, request, `O${this.seq}`, "set", function(err, response) {
    if (err) {
...
Client.prototype.add = function(key, value, options, callback) {
  ...
  var serialized = this.serializer.serialize(opcode, value, extras);
  var request = makeRequestBuffer(opcode, key, serialized.extras, serialized.value, this.seq);
  this.perform(key, request, this.seq, "add", function(err, response) {
    if (err) {
      if (callback) { callback(err, null, null); }
      return;
    }
...
Client.prototype.replace = function(key, value, options, callback) {
  ...
  var serialized = this.serializer.serialize(opcode, value, extras);
  var request = makeRequestBuffer(opcode, key, serialized.extras, serialized.value, this.seq);
  this.perform(key, request, this.seq, "replace", function(err, response) {
    if (err) {
      if (callback) { callback(err, null, null); }
      return;
    }
...
Client.prototype.delete = function(key, callback) {
  ...
  let request = "md " + key + " O" + this.seq+ "\r\n"
  this.perform(key, request, `O${this.seq}`, "delete", function(err, response) {
    if (err) {
      if (callback) { callback(err, null, null); }
      return;
    }
...
Client.prototype.increment = function(key, amount, options, callback) {
  ...
  this.perform(key, request, this.seq, "increment", function(err, response) {
    if (err) {
      if (callback) { callback(err, null); }
      return;
    }
...
Client.prototype.decrement = function(key, amount, options, callback) {
  ...
  this.perform(key, request, this.seq, "decrement", function(err, response) {
    if (err) {
      if (callback) { callback(err, null); }
      return;
    }
...
Client.prototype.append = function(key, value, callback) {
  ...
  var request = makeRequestBuffer(opcode, key, serialized.extras, serialized.value, this.seq);
  this.perform(key, request, this.seq, "append", function(err, response) {
    if (err) {
      if (callback) { callback(err, null); }
      return;
    }
...
Client.prototype.prepend = function(key, value, callback) {
  ...
  var serialized = this.serializer.serialize(opcode, value, '');
  var request = makeRequestBuffer(opcode, key, serialized.extras, serialized.value, this.seq);
  this.perform(key, request, this.seq, "prepend", function(err, response) {
    if (err) {
      if (callback) { callback(err, null); }
      return;
    }
...
Client.prototype.touch = function(key, expires, callback) {
  ...
  this.perform(key, request, this.seq, "touch", function(err, response) {
    if (err) {
      if (callback) { callback(err, null); }
      return;
    }
...
Client.prototype.perform = function(key, request, seq, func, callback, retries, ) {
  var _this = this;
  ...
  var responseHandler = function(response) {
    ...
    // console.log("endResponseHandler")
  ...
  var errorHandler = function(error) {
    if (--retries > 0) {
      ...
      setTimeout(function() {
        _this.perform(key, request, seq, func, callback, retries);
      }, 1000 * retry_delay);
    } else {
      ...
      if (failover) {
        ...
        _this.perform(key, request, seq, func, callback, origRetries);
      } else {
  ...
  serv.onResponse(seq,func, responseHandler);
